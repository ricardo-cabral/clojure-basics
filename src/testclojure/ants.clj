...
(defn bound
  "return n wrapped into range 0-b"
  [b n]
    (let [n (rem n b)]
      (if (neg? n)
        (+ n b)
...
(defn wrand
  "given a vector of slice sizes, return the index of a slice given a
...
(defn move
  "moves the ant in the direction it is heading. Must be called in a transaction
  that has verified the way it is clear"
  [loc]
    (let [oldp (place loc)
        ant (:ant @oldp)
...
(defn take-food [loc]
  "takes one food from current location. Must be called in a transaction
       ...
       (dosync
         ...
         (if (:food ant)
           ...
           (cond
             ...
             (move loc)
             :else
             (let [ranks (merge-with + (rank-by (comp :food deref) places)
                                     (rank-by (comp :pher deref) places))]
               (([move #(turn % -1) #(turn % 1)]
                 (wrand [(if (:ant @ahead) 0 (ranks ahead))
                         (ranks ahead-left) (ranks ahead-right)]))
...
(defn render-ant [ant #^Graphics g x y]
  (let [black (. (new Color 0 0 0255) (getRGB))
...
(defn render [g]
  (let [v (dosync (apply vector (for [x ( range dim) y (range dim)]
        ...
        bg (. img (getGraphics))]
    (doto bg (.setColor (. Color white))
             (.fillRect 0 0 (. img (getWidth)) (. img (getHeight))))
    (dorun
      (for [x (range dim) y (range dim)]
        (render-place bg (v (+ (* x dim) y)) x y)))
    (doto bg (.setColor (. Color blue))
...
(def panel (doto (proxy [JPanel] []
                         (paing [g] (render g)))
             (.setPreferredSize (new Dimension (* scale dim)
                                     (* scale dim)))))
